...
> `summary`(줄거리)는 아직 생성하지 않았으면 `null`. 예전 버전이 저장한 깨진 값(JSON 원문)도 `null`로 내려보내 앱이 자동 재생성을 태우게 한다. **목록(`GET /dreams`)에는 포함하지 않는다** — E1에서 일부러 컬럼을 줄인 곳이고 줄거리는 상세에서만 쓰인다.
...
```
- **멱등**: 이미 줄거리가 있으면 Gemini를 부르지 않고 기존 값 + `cached: true` 반환. 단 **저장된 값이 깨져 있으면(JSON 원문이 그대로 들어간 값) 캐시로 인정하지 않고 다시 생성**한다 (2026-08-29)
...
- **깨진 값은 저장하지 않는다** (2026-08-29 사고 대응). Gemini가 JSON을 끝맺지 못하고 끊는 경우가 있는데, 예전에는 그 원문을 그대로 줄거리로 저장해 `{ "summary": "...` 가 사용자 화면에 노출됐다. 지금은 ① `response_schema`로 응답 형식을 강제하고 ② 잘린 JSON은 닫는 괄호를 채워 복구하고 ③ 그래도 실패하면 재시도, ④ 끝내 실패하면 저장 없이 503으로 끝낸다
