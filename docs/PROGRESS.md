...
> 최종 업데이트: 2026-08-16 (**대화 구조 전면 개편 + 줄거리 기능 + 버그 3건, 서버 배포 완료**. 꿈 기록 대화를 턴 카운터에서 **슬롯 채우기**로 전환 — 한 번에 다 말하면 1턴에 끝나고 되묻지 않는다(프로덕션 실증). S-2 줄거리 API 신설·배포. B9-1·2·3·4·5·6·7 + A1 최소수정·A2로 **build 9 앱 코드가 전부 모임**. 꿈 1건 비용 **₩14.4 → ₩11.35**. 하위 호환 설계라 서버 선배포했고 스토어의 build 8도 정상 동작 확인. **남은 것: EAS 빌드 → TestFlight 인증 회귀 3종 + 줄거리/토글 검증 → 심사 제출**)
>
