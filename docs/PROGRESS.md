...
> 최종 업데이트: 2026-08-08 (**심사 착수 → Guideline 2.1(b) 정보 요청 수신 → 답장 회신 완료**. 7/8 제출한 build 8이 드디어 심사 진입(심사일 8/8, iPad Air 11" M3). 지난 반려 5개 항목은 **전부 해소**되었고, 유일한 쟁점은 "IAP 없이 유료 등급이 있어 보인다". 약관 제11조 개정 배포 완료 + 영문 답장 게시 완료 + 앱 내 Premium 표기 제거 코드 반영 완료(빌드는 미생성). **다음은 심사관 회신 대기 — 2~3일, 8/12(수)까지 무응답 시 build 8 재제출**)
> 대상 위치: `dreamteller/app/` (Expo) + `dreamteller/server/` (FastAPI) + `dreamteller/web/` (Amplify 정적 사이트) + `dreamteller/app-store/` (스토어 제출용 산출물)

---

## 오늘 세션 요약 (2026-08-08, App Review 2.1(b) 정보 요청 대응) 📮
