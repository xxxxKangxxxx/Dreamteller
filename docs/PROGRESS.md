...
0. **실기기 스토어 버전 스모크** ⏭️ **다음 세션으로 이월(사용자 결정)** — **스토어 `1.1.0` 다운로드는 완료**돼 있으므로 앱을 열어 **기록 → 줄거리 → 해몽** 1회만 돌리면 된다. TestFlight build 10과 동일 바이너리지만 스토어 경로는 아직 안 밟아봤다
1. **A1 전체 리팩터** — 미러 제거 + `getSession()` 전환 + C1. 인증 회귀 3종과 같은 사이클로. warm resume 최소 수정은 검증됐으므로 급하지 않다
2. **2-1 Archive 백엔드** — 줄거리 프롬프트를 확장해 인물·장소 추출 되살리기
3. **S-3 FREE 한도 재설계 후속** — 실사용 로그(`call=summary`) 쌓인 뒤 재검토
4. **1-8 EC2 스왑 2GB** — 10분 작업, OOM 시 진행 중 해몽 잡 유실 경로
5. **메타데이터 별도 사이클** — 키워드 실제값 48/100자, 기능 업데이트와 섞지 않기로 한 건
6. *(선택)* **게스트 계정 warm resume** — 코드 경로가 동일해 우선순위 낮음. 하더라도 **일회용 게스트**로 할 것(실패 시 그 계정은 영구 상실)
