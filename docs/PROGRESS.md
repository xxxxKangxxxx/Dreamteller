...
- ⏳ **배포 미완** — 작업 공간 외부라 SSH가 막혀 있다(SG에 기본 작업 공간 IP만 열림). 커밋·푸시만 해두고 **다음 세션에 `git pull` + `systemctl restart`**
...
0. ⏳ **줄거리 문단 프롬프트 배포** — 코드는 커밋·푸시 완료, EC2 반영만 남았다. **기본 작업 공간에서** `git pull` + `systemctl restart` (SSH가 그 IP만 열려 있음). 배포 후 새 꿈 1건으로 문단이 실제로 나뉘는지 확인
1. **실기기 스토어 버전 스모크** ⏭️ **다음 세션으로 이월(사용자 결정)** — **스토어 `1.1.0` 다운로드는 완료**돼 있으므로 앱을 열어 **기록 → 줄거리 → 해몽** 1회만 돌리면 된다. TestFlight build 10과 동일 바이너리지만 스토어 경로는 아직 안 밟아봤다. **위 0번 배포 후에 하면 문단 확인까지 한 번에 끝난다**
2. **A1 전체 리팩터** — 미러 제거 + `getSession()` 전환 + C1. 인증 회귀 3종과 같은 사이클로. warm resume 최소 수정은 검증됐으므로 급하지 않다
