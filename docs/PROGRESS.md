...
> 최종 업데이트: 2026-08-17 (🎉 **`1.1.0 (10)` 심사 통과 → 출시 → 스토어 라이브 + A1 검증 통과**. 8/16 22:27 제출 → **하루 안에 승인**(첫 출시는 50일). 반려·정보요청·추가빌드·Expedite **전부 0회**. 수동 출시 → **14:47 KST 전파 완료**. build 8에 9일간 노출돼 있던 **`FREE` 뱃지 / 구독 UI 없는 "프리미엄 알아보기" 막다른 길 버튼 소멸**. **A1 warm resume 최종 검증 통과** — 70분 백그라운드 후 warm resume + 분석 탭 첫 진입(스피너=실요청)에서 로그아웃 없음. 어제 남은 마지막 미검증 경로가 닫혔다. 서버 정상(`/health` 200, `summary` 라우트 401=존재). **남은 것: 스토어 버전으로 실기기 스모크 1회**)
>
...
0. **실기기 스토어 버전 스모크 — 나머지 경로** — 줄거리 경로는 문단 확인하며 밟았으므로, **해몽까지 이어서 1회**만 확인하면 스토어 바이너리 검증이 닫힌다(TestFlight build 10과 동일 바이너리지만 스토어 경로는 미검증)
