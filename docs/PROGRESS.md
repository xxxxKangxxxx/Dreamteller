...
꿈 상세 **줄거리** 탭에 아래가 그대로 노출됐다.

```
{
  "summary": "나는 노트북으로 내 방에서 취업 컨설팅을 받고 있었다. … \n\n그러다 갑자기 …
```

프로덕션 DB 실측 (`dreams.summary`, id `bcbec3aa…`):

| 확인 | 결과 |
|---|---|
| 저장값 | **1326자**, `{\n  "summary": "…` 로 시작 |
| 끝부분 | 본문은 온전한데 **닫는 `"` 와 `}` 가 없음** |
| `json.loads` | `Unterminated string starting at: line 2 column 14` |

**원인** — Gemini가 JSON을 끝맺지 못하고 끊은 응답을, 폴백이 **원문 그대로 줄거리로 사용**(`gemini_service.py` 구 `475-480`)하고 그 값을 DB에 저장했다.

추측을 배제하려고 실측으로 좁혔다.

| 가설 | 검증 | 판정 |
|---|---|---|
| 출력 토큰 상한 초과 | 깨진 응답 **725토큰**, 모델 상한 **65k**. 잘린 지점도 라운드 넘버 아님 | ❌ |
| 특정 입력에서 항상 재현 | 같은 `chat_history`로 재호출 → `finish=STOP`, **정상 JSON**(728토큰) | ❌ |
| 확률적 형식 붕괴 | 본문 다 쓰고 **마지막 2글자만** 빠뜨림 | ✅ |

→ 출력이 길수록 빠뜨릴 구간이 넓어진다. **긴 줄거리에서 더 잘 터진다.**

**한 번 저장되면 낫지 않는 구조였다** — `POST /dreams/{id}/summary`가 멱등이라 캐시값을 그대로 돌려주고(`dreams.py:154`), 앱도 `summary`가 있으면 재요청하지 않는다(`InterpretScreen.tsx:95`).

**같은 폴백 패턴이 2곳 더 있었다** — `chat_turn`(깨진 JSON을 `reply`로 노출), `generate_interpretation`(원문을 `symbolAnalysis`에 투입). 아직 안 터졌을 뿐 구조가 동일해서 함께 고쳤다.

**로그 사각지대** — 줄거리 경로만 `finish_reason`을 안 남기고 있어서, 이 사고가 서버 로그에는 경고 한 줄로만 보였다.

### 2. 수정 — 3층 방어

**① 원문 노출 폴백 제거 + 잘린 JSON 복구** — `server/app/utils/model_json.py` 신설

- `parse_model_json()`: 그대로 파싱 → 실패 시 **잘림인지 판정**해 닫는 따옴표/괄호만 채워 1회 복구 → 그래도 실패면 `None`
- 복구는 **잘림에만** 적용. 구조가 어긋난 응답(`{"a":1}}`)은 복구가 아니라 재시도 대상
- 반쪽 이스케이프(`\`로 끝남), 매달린 콤마/콜론, 코드펜스(` ```json `)까지 처리
- **원문을 되살리는 폴백은 3곳 모두에서 삭제.** 최종 폴백은 줄거리=빈 문자열(→503), 해몽=`RuntimeError`, 대화=슬롯 폴백 질문
- 파싱 실패 시 재시도 추가. **백오프는 넣지 않았다** — 서버 과부하가 아니라 형식만 깨진 응답이고, 줄거리는 사용자가 화면에서 기다리는 동기 호출이라 1.5초씩 까먹을 이유가 없다

**② `response_schema` 적용** (줄거리) — `response_mime_type`만 주는 건 "JSON처럼 써줘" 수준이지만, 스키마를 주면 **디코딩 자체가 제약**돼 잘림·형식 붕괴가 줄어든다. 이번 사고의 1차 방어선.

> 대화(chat)에는 넣지 않았다. 슬롯이 `string | null` 구조라 스키마화 시 nullable 처리에서 회귀 위험이 있어, 복구 + 재시도만으로 충분하다고 판단.

**③ 저장 전 검증 + 읽기 시 자가 복구** — `routes/dreams.py`

- `looks_like_raw_json()` 통과 못 하면 **DB에 넣지 않고 503**. 한 번 저장되면 영구 노출되는 게 이번 사고의 핵심
- `GET /dreams/{id}`가 깨진 값을 `null`로 내려보내 → 앱 자동 생성 경로 재진입 → `POST …/summary`도 깨진 값을 캐시로 인정하지 않음. **배포만으로 기존 깨진 데이터가 스스로 낫는다**
- 줄거리 경로에 `finish_reason` 로깅 추가 (사각지대 해소)

**④ 기존 데이터 복구** — `server/scripts/heal_broken_summaries.py` (dry-run 기본, `--apply`로 반영)

전체 26건 중 깨진 1건. **Gemini 재호출 없이** 닫는 괄호만 채워 복구 → 1326자 → **1304자**(JSON 껍데기 22자만 제거, `\n\n`도 실제 줄바꿈으로 복원, **본문 손실 0**). 원문은 `server/scripts/backup/`에 백업(git 미포함).

### 3. 배포 & 검증

`d52b00e` → EC2 `git pull --ff-only` → `systemctl restart` → `active`, 기동 로그 에러 없음.

| 검증 | 결과 |
|---|---|
| `/health` | 200 |
| `/api/dreams`, `/api/dreams/{id}` | 401 (인증 요구 = 라우트 정상) |
| `/api/dreams/{id}/summary` | 405 (POST 전용 = 정상) |

**실사용 검증** — 긴 줄거리 + 장면 전환 2회 + 큰따옴표 대사 2개(이스케이프가 가장 잘 깨지는 자리)로 구성한 꿈을 새로 기록.

```
gemini usage call=summary attempt=1 prompt=689 output=284 total=973 usd=0.000917
summary generated chars=496 finish=FinishReason.STOP attempt=1
```

| 항목 | 결과 |
|---|---|
| `attempt=1` | 복구·재시도 없이 1회 성공 |
| `finish_reason` 로깅 | 정상 출력 (신규) |
| JSON 껍데기 노출 | **False** |
| 리터럴 `\n` 노출 | **False** (실제 줄바꿈 저장) |
| 문단 나누기 | 2문단 — 장면 전환 반영 |
| 큰따옴표 대사 | 4개 온전 |
| 해몽 경로 | `call=interpret attempt=1` 성공, `201 Created` — 파서 교체 회귀 없음 |

### 4. 오늘 발생한 오류 및 해결 내역

| # | 증상 | 원인 | 해결 |
|---|---|---|---|
| 1 | heal 스크립트 실행 시 `zsh: permission denied: /private/tmp/…/heal.py` | 긴 스크래치패드 경로를 붙여넣는 과정에서 **줄바꿈으로 명령이 쪼개져** `heal.py`가 직접 실행됨(644라 실행권한 없음). 권한 문제가 아니었음 | 스크립트를 리포 안(`server/scripts/`)으로 옮겨 **한 줄 명령**으로 축소 |
| 2 | 배포 검증 시 `/dreams` → 404 | 라우터 prefix가 `/api/dreams` (`main.py:66`) | 정확한 경로로 재확인 → **401**(정상) |
| 3 | 프로덕션 DB 쓰기 / `app.main` 임포트가 auto mode 분류기에 차단 | 자격증명 로딩 + 프로덕션 쓰기 | 읽기 검증은 Claude, **쓰기는 사용자가 실행**. 임포트 검증은 `py_compile` + 가짜 클라이언트 단위 테스트로 대체 |
| 4 | 리포 루트에서 `./venv/bin/python` → not found | Bash 도구는 **작업 디렉터리가 유지**되는데 상대경로를 `server/` 기준으로 씀 (08-20 이력 #4와 동일 함정) | 절대경로 또는 매 명령에서 `cd` 명시 |

### 5. 검증 방식 메모

프로덕션 DB 쓰기가 막힌 상황에서 코드 검증을 이렇게 대체했다 — 다음에 재사용 가능:

- **파서 단위 테스트 16케이스** — 잘림 7종(닫는 `"}` 누락 / `}`만 누락 / 코드펜스 / 개행 이스케이프 / 따옴표 포함 / 반쪽 이스케이프 / 매달린 콤마·콜론) + 복구 불가 4종 + 저장 방어선 3종
- **가짜 Gemini 클라이언트**로 `generate_summary` 3경로 — 잘림→1회 복구 / 실패→2회째 성공 / 3회 전부 실패→빈 문자열
- 실 API 호출은 **스키마 동작 확인용 1회만** (₩1.3)

### 6. 다음 작업 (우선순위)

1. **build 11 묶음** — 줄거리 문단 간격 정교화 + A1 전체 리팩터 (**여전히 최우선**)
2. **i18n 착수** — [`docs/I18N_PLAN.md`](./I18N_PLAN.md) §11 재조사 명령부터
3. (관찰) chat 경로 `response_schema` 적용 — 이번엔 회귀 위험으로 보류. 대화에서 JSON 깨짐 로그(`chat JSON 잘림 감지`)가 실제로 쌓이면 재검토

---

## 이전 세션 요약 (2026-08-20, 영어 지원(i18n) 실행 계획 수립) 🌐

> **코드 변경 없음.** 조사 · 설계 결정 · 문서화만. 착수는 추후.
> 산출물: [`docs/I18N_PLAN.md`](./I18N_PLAN.md) (1,264줄) + `CLAUDE.md` 문서 목록 1줄

### 1. 현황 확인 — 한국어 전용이 맞다

전수 조사로 확정. i18n 구조가 **아예 없다** (라이브러리 0개, 로케일 디렉터리 없음, 기기 언어 감지 코드 없음).

| 영역 | 물량 |
|---|---|
| 앱 UI 한글 문자열 | **27파일 / 249줄** (주석 제외) |
| 서버 프롬프트 | `gemini_service.py` 7종 전부 `언어: 한국어` 고정 |
| 서버 사용자 노출 문자열 | `main.py:57` **단 1줄** (나머지는 `DREAM_NOT_FOUND` 영문 코드) |
| 웹 | `index/privacy/terms.html` 전부 `lang="ko"` |

### 2. 설계 결정 10개 — 사용자와 논의 후 확정

| # | 결정 |
|---|---|
| ① | UI + AI 생성 콘텐츠 **전부** 영어화 |
| ② | **한국 전통 해몽 유지** — 영어권 차별화 포인트 |
| ③ | 언어 선택 `시스템 / 한국어 / English` 3지 |
| ④ | **기록의 언어를 따른다** — 한국어로 쓴 꿈은 UI가 영어여도 한국어 해몽 |
| ⑤ | 문화 맥락은 `keySymbols.meaning`에 인라인 (**DB payload 구조 불변**) |
| ⑥ | 영어 출력 길이 = 한글:영어 1:2.5 환산 (카드 레이아웃 고정) |
| ⑦ | 카드 2단 라벨 유지 — `keyword` 공통 / `sub`만 로케일 |
| ⑧ | Luna 영어 톤 = 친구 톤 (축약형·구어체) |
| ⑨ | **1.2.0 한 번에** 출시 (EAS 빌드 1회 + 심사 1회) |
| ⑩ | 영문 약관은 초안 → 사용자 검토 → 반영 |

### 3. 사전 검증에서 나온 것 (추측 아닌 실측)

**⚠️ Hermes에 `Intl.PluralRules`가 없다** — iOS Hermes 바이너리 심볼을 직접 조사한 결과:

| API | |
|---|---|
| `Collator` / `DateTimeFormat` / `NumberFormat` | ✅ 있음 |
| **`PluralRules`** | ❌ **없음** |
| `RelativeTimeFormat` / `ListFormat` / `Segmenter` | ❌ 없음 |

i18next v23+ 는 복수형 판정에 `Intl.PluralRules`를 쓴다 → **`@formatjs/intl-pluralrules` 폴리필 없으면 `"1 dreams"` 버그.**
날짜·숫자는 `Intl.DateTimeFormat`이 있어서 그대로 된다.

**⚠️ 번역하면 깨지는 로직 3곳** — 한글 에러 메시지를 값으로 비교한다.
```
SettingsScreen.tsx:120  /  LoginScreen.tsx:77, :92
  if (message !== '로그인을 취소했어요')
```
→ i18n과 무관하게 그 자체로 옳은 수정이라 **Phase 1 선행 리팩터**로 분리 (단독 배포 가능).

**⚠️ 문장 조립 함정 5곳** — 한국어는 조사가 뒤에 붙어 `{변수}+고정문구` 조립이 되지만 영어는 어순이 달라 깨진다.
`OtpVerifyScreen:105~108`(변수가 뒤로), `InterpretScreen:353~357`(5조각 결합), `InsightsScreen:27`(순서 반대) 등.
→ **"번역 키는 문장 단위, JSX 조각 결합 금지"**를 규약으로 명시.

**⚠️ 12시간제** — `SettingsScreen:52`의 `오전 8:00`(접두) ↔ `8:00 AM`(접미). 문자열 치환으로 안 되고 포맷이 뒤집힌다.

**좋은 소식** — 서버 에러 i18n은 거의 공짜(1줄), `emotion.ts`는 키가 enum이라 DB 변경 불필요,
`splitIntoParagraphs()`는 영문도 그대로 동작, 줄거리 프롬프트는 원래 분량 제한이 없어 길이 환산 불필요.

### 4. `docs/I18N_PLAN.md` 구성 (1,264줄)

나중에 다른 세션이 **이 문서만 보고 실행**할 수 있도록 작성했다.

| § | 내용 |
|---|---|
| 0 | 착수 시점 스냅샷 (버전·SDK·물량 — 나중에 대조용) |
| 1~4 | 결정 10개 / 제약 7개 / 검증된 사실 / 번역 키 규약 |
| 5 | **Phase 1~8** — 커밋 단위까지 분할, 코드 스켈레톤 포함 |
| 6~7 | 영어 프롬프트 사양(길이 환산표·톤·문화맥락) / 주요 문자열 대응표 |
| 8~10 | QA 체크리스트 / 리스크 12건 / DoD 7항목 |
| **11** | **재조사 명령 모음** — 줄번호가 밀릴 것에 대비해 grep 7세트 + 기준 결과값 |
| **12** | **영어 프롬프트 전문** — Luna / 해몽 / 제목 / 요약 7종 초안 |
| **13** | **문자열 전체 인벤토리** — 27파일 249줄을 `파일:줄 / 한국어 / 영어` 표로 |

### 5. 오늘 발생한 오류 및 해결 내역

| # | 증상 | 원인 | 해결 |
|---|---|---|---|
| 1 | `strings ... \| grep -cE "^(DateTimeFormat\|...)$"` → 전부 0 | `strings` 출력은 긴 문자열 덩어리라 **`^...$` 앵커가 매칭될 수 없음** | 앵커 제거하고 부분 문자열로 grep → `Intl.DateTimeFormat` 등 24건 확인 |
| 2 | Hermes 심볼 조사 첫 시도가 **tvOS 슬라이스**를 읽음 | `find ... -name "hermes" \| head -2`가 아키텍처 무관하게 잡음 | `-path "*ios-arm64/hermes.framework/hermes"`로 슬라이스 지정 |
| 3 | 한글 문자열 카운트가 주석까지 포함 | 필터가 `//`, `*`만 제외하고 **`/*`(JSDoc 시작줄) 누락** | `':\s*(//\|\*\|/\*)'`로 보정 → 249줄로 확정 |
| 4 | `cd app/src` 후 다음 명령에서 `no such file or directory` | Bash 도구는 **작업 디렉터리가 유지**되는데 상대경로를 매번 루트 기준으로 씀 | 절대경로 사용 또는 매 명령에서 `cd` 명시 |
| 5 | 계획 문서 heredoc이 `docs/I18N_PLAN.md`를 못 찾음 | `cd /Users/.../DreamTeller`로 갔는데 실제 리포는 **한 단계 아래** `DreamTeller/dreamteller/` | 절대경로 변수(`P=...`)로 고정. **기존 파일은 리다이렉트 대상 경로가 무효라 훼손되지 않았음** |

---

## 이전 세션 요약 (2026-08-18, Higgsfield 연결 + UGC 광고 영상 제작) 🎬

> 앱·서버 코드 변경 **없음**. 마케팅 산출물 작업.

### 1. Higgsfield 설정 — 대부분 이미 돼 있었다

CLI(`@higgsfield/cli@1.1.23`)와 스킬 8종은 **이미 설치돼 있었고**, 실제로 빠진 건 인증뿐이었다.
로그인 후에도 `account status`가 실패했는데 원인은 **워크스페이스 미선택**이었다 — `Private` 하나뿐이라 그걸로 지정.

```
higgsfield workspace set d2679f77-79eb-42ed-a67c-b92f5635dc75
→ kang071911@gmail.com — plus plan
```

### 2. 광고 영상 — 11초 세로형 UGC

`ugc-saas-flow` 워크플로우 채택. **핵심 이유는 "실제 캡처 픽셀만 사용, AI의 UI 생성 금지"가 이 워크플로우의 하드룰**이라는 점이다. AI에게 앱 화면을 그리게 하면 한글이 깨지고 없는 기능이 나온다.

| | 내용 |
|---|---|
| 스펙 | 1080×1920 · 24fps · 11.04초 · H.264 · **무음** |
| 카드 | `screenshots/raw/` 실제 캡처 3종 (홈 / Luna 대화 / 꿈 해몽) |
| 자막 | 꿈은 5분이면 사라져요 → 대화하면서 기록하는 꿈 일기 → DreamTeller / 지금 앱스토어에서 만나보세요 |
| 비용 | 99 크레딧 (영상) + 0.12 (크리에이터 이미지) |

> ⚠️ **이 영상은 App Store 프리뷰로 쓸 수 없다.** Apple의 앱 프리뷰는 실제 앱 화면 캡처만 허용하며 인물이 등장하는 광고는 거부된다. **인스타 릴스·틱톡 등 소셜 광고용**이다.

### 3. 오늘 발생한 오류 및 해결 내역

| # | 증상 | 원인 | 해결 |
|---|---|---|---|
| 1 | 30초 1080p 견적이 **270 크레딧** (보유 110) | 1080p는 **9 크레딧/초**, 720p 4.5, fast 3.5 | 길이를 11초로 축소(99). 생성 전 `get_cost: true` 프리플라이트를 습관화 |
| 2 | `account status` → `No workspace selected` | 로그인만으로는 부족 | `workspace set` |
| 3 | 샌드박스 자막이 **두부(□)로 깨질 위험** | 한글 폰트 부재 — WenQuanYi=중국어, Unifont=비트맵, 기본 폰트 Metropolis/Montserrat=라틴 전용 | Noto Sans KR 직접 다운로드 후 **29자 글리프 렌더링 검증** 통과시키고 진행 |
| 4 | Noto Sans KR **static Bold 404** | 구글 폰트 리포에 **가변 폰트만 존재** | PIL `set_variation_by_axes([700])` 로 굵기 지정 |
| 5 | 글리프 검증용 `fontTools` 없음 | 샌드박스 미설치 | PIL로 문자별 비트맵을 렌더해 **해시 중복·잉크 유무**로 판정하는 방식으로 대체 |
| 6 | 프리셋 **"IN THE DARK"** 자동 추천 | 프롬프트 오매칭 | 밝은 아침 침실과 정반대라 `declined_preset_id`로 거절 |
| 7 | 로컬 PIL 없음 | — | 사용자 환경 오염 없이 **scratchpad venv**에 설치 |

**워크플로우에서 의도적으로 벗어난 지점 1건** — de-slop(피부 보정, 3크레딧)을 건너뛰었다. 그 단계는 Soul이 피부를 밀랍처럼 뭉갰을 때 고치는 용도인데 시드 이미지에 그 문제가 없었고, 재생성 시 얼굴이 바뀔 위험이 있었다.

### 4. 산출물 — `app-store/ad/` (git 미포함)

| 파일 | 비고 |
|---|---|
| `dreamteller-ugc-11s.mp4` | 최종본. `recompose.sh`로 **md5 동일하게 재생성됨을 검증** |
| `dreamteller-ugc-11s-raw.mp4` | 합성 전 원본 — 99크레딧짜리, **재생성 불가** |
| `creator-seed.png` | 크리에이터 시드 — **없으면 같은 인물로 클립 추가 불가** |
| `overlays/`, `build_assets.py`, `recompose.sh` | 자막·타이밍 수정 시 **크레딧 0으로** 재합성 |

29MB 바이너리라 **git에는 올리지 않는다**(사용자 지시). 추적 파일 총량이 16.8MB인 리포에 히스토리 영구 부담을 주지 않기 위함이기도 하다. 루트 `.gitignore`에 `app-store/ad/` 추가.

> **로컬에만 있다는 뜻이므로 백업은 사용자 책임.** 특히 `-raw.mp4`와 `creator-seed.png`는 유실 시 크레딧을 다시 써야 복구된다.

### 5. 다음 작업 (우선순위)

1. **build 11 묶음** — 줄거리 문단 간격 정교화 + A1 전체 리팩터 (지난 세션 이월, 변동 없음)
2. 크레딧 충전 여부 판단 — 30초 풀버전 광고는 **270 크레딧**. 현재 잔액 10.88
3. 11초 광고 실사용 — 인스타 릴스/틱톡 업로드 (App Store 프리뷰로는 사용 불가)

---

## 이전 세션 요약 (2026-08-17, 🎉 `1.1.0` 심사 통과 → 출시) ⭐⭐
