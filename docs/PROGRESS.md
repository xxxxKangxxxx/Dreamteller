...
- ⏳ build 3 잔여 검증: iOS 자동채움 / 재발송 쿨다운 / 별빛 로더(#2) / 토스트(#3) — 진행 중
- 🆕 **이슈 4 발견** ↓

| 4 | 🟡 폴리시 | `OtpVerifyScreen` | OTP 자동 인증 후 홈 전환이 너무 즉각적(깜빡) | 인증 중 ActivityIndicator "인증하고 있어요..." 로딩 표시 + 최소 노출 800ms 보장. tsc 통과, **다음 빌드 대기** |

**미검증(다음 빌드 후)**: iOS OTP 자동채움, 재발송 쿨다운 60s, 별빛 로더(#2), 토스트(#3), OTP 로딩(#4)
> 빌드 절약: build 3 잔여 항목 마저 검증 → 추가 이슈 있으면 #4와 묶어 **다음 빌드 1회**로 처리 (잔여 2회)
