...
**운영 메모**
- `_log_usage`에 `call=summary`로 기록 → S-1 로그로 실단가를 분리 측정한다
- **thinking은 일단 기본값 유지.** 제목(§4)처럼 끄기 전에 실측이 먼저다. 줄거리는 재구성 작업이라 추론 여지가 적을 것으로 보이나, 150~300자 서술은 구성이 필요하므로 `thinking_budget=0`보다 **제한(512 등) 후 품질 비교**가 맞다 (`IMPROVEMENTS.md` 1-1c와 같은 접근)
