...
- 분량 제한은 없다. 짧은 꿈은 짧게, 긴 꿈은 길게 — 대화에 담긴 만큼만 쓸 것.
  분량을 맞추려고 없는 내용을 지어내는 것이 가장 나쁘다.
- 장면이 바뀌거나 시간이 흐르면 문단을 나눌 것. 문단과 문단 사이는 빈 줄 하나로 구분한다.
  다만 문단 수를 억지로 맞추지 말 것 — 짧은 꿈은 한 문단으로 두는 것이 자연스럽다.
